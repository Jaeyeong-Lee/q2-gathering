...
| 1-5 | 과제 별이 PJT 칸(8개면 4×2)으로 해쳐모임 → 한 PJT에 들어가 닮은 과제끼리 성운 | wow5 | 엔진 장면 2 |
| 1-6 | 과제 하나의 확보/필요 역량 (결정 4에 따라 생략 가능) | | 엔진 장면 3 |
| 1-7 | 맺음 — "우리의 미래 경쟁력이 팀의 로드맵" | | 스토리 레이어 |
...
4. **역량은 선택.** 실데이터 품질이 끝내 안 나오면 1-6·2-4에서 뺀다. T5 품질 점검으로 판단.
...
| 스토리 레이어 | 켜짐 (1-0·1-1·1-2·1-7) | 꺼짐 |
| 합성 표시 | 항상 | 없음 |

**엔진 ↔ 스토리 레이어 계약.** T3와 T4가 병렬로 일하기 위한 유일한 접점이다. 바꾸려면 이 블록을 먼저 고치고 두 티켓에 알린다.

```js
window.nebula = {
  go(sceneIndex),            // 4.4의 새 번호
  focusPerson(personIndex),  // 줌 → 자동 분열. 끝나면 resolve하는 Promise
  clearPerson(),             // 줌 아웃. Promise
  enterPjt(pjtIndex),        // 장면 2 격자 → 그 PJT로 진입. 성운이 모이면 resolve하는 Promise
  leavePjt(),                // 장면 2 격자로 복귀. Promise
  selectTask(taskId),
  peoplePositions(),         // 장면 0 사람의 화면 좌표 [{id, x, y}] (px)
};
```

- `go(1)`·`go(2)`의 전환이 끝나면 resolve하는 Promise를 돌려준다(스크립트가 다음 비트를 기다리기 위해).
- 스토리 레이어는 `#sky` 위 별도 레이어에 그린다. 1-2의 마지막 프레임에서 별 위치를 `peoplePositions()`에 맞춘 뒤 사라져, 엔진 장면 0과 이음매가 보이지 않게 한다.
- 파일 `nebula/templates/story.js`. `render.py`가 `__STORY__` 자리에 인라인한다(`__PAYLOAD__`와 같은 방식).
...
### T4 스토리 레이어: 1-0·1-1·1-2·1-7과 1부 스크립트 — 에이전트, `story.js` 단독
...
- **완료**: 1920×1080 헤드리스로 비트마다 스크린샷 1장(총 8장)을 만들어 공유(커밋하지 않음); 1-2 → 장면 0 이음매에서 별 위치가 튀지 않음; `prefers-reduced-motion`에서 이동 대신 페이드; 합성 표시 상시.
