...
### 운영 원칙 — 에이전트는 "실행하고 결과만 보는" 위치

**실행은 에이전트가 한다.** 금지되는 건 실행이 아니라 산출물을 직접 열어보는 것이다.
...
콘솔이 안전하도록 파이프라인 전 구간에서 원문·실명을 뺐다 — `td_extract`(추출 폐기),
`normalize_person_text`(정제 완료/실패), `merge_persons`(제외), `split_md_by_person`(이상)
전부 실명 대신 **`seq`/`id`**만 찍는다. 실명은 ECS 로그(`TD_OUT_DIR` 안)에만 남으니
누가 문제인지는 사람이 seq로 DB를 조회하면 된다.

```bash
sqlite3 data/roster.db "SELECT name FROM roster WHERE seq IN (12, 45)"
```

산출물을 사람이 검수하다 이상한 걸 발견하면, 파일을 붙여넣지 말고 **무엇이 어떻게 이상한지
말로** 설명해라 — 지난번 `relations` 문제도 그렇게 해서 원문 없이 고쳤다.
