...
- **`data/td_sample/`은 열어도 되는 유일한 td 산출물 디렉터리다.** `make td-sample`이
  합성 코퍼스로 파이프라인을 완주해 산출물·로그·콜덤프를 전부 거기에 남긴다 — 입력이
  가짜라 안에 실데이터가 없다. 화면을 만들 땐 마음껏 읽어라. 그 바깥은 여전히 금지고,
  `data/task_discovery/`도 마찬가지다.
