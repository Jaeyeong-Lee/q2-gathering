...
status: done # archive/cloud_logic.js + wordcloud2.min.js 벤더링 + 템플릿 패널, 헤드리스 크롬 검증 완료
blocked_by: [001]
...
- wordcloud2.js (캔버스 기반, 인라인 포함) 사용
...
- 절대 횟수는 표시하지 않고 상대 크기만 노출 (Higher/Lower 게임 스포일러 방지)
...
- [x] 클라우드 어디에도 절대 횟수가 노출되지 않는다 (cloudState가 최대값 대비 상대 크기만 반환, 테스트로 고정)
